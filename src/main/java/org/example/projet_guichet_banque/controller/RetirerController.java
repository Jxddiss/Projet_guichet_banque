...
public class RetirerController {
    @FXML
        ...
        if (actionEvent.getSource() == confirmerBtn){
            try{
                ...
                if (AffichageCompteController.compteChoisi.getType().equals("cheque")){
                    solde = LoginController.gestionnaireGuichet.retraitCheque(montant,AffichageCompteController.compteChoisi.getNumeroCompte());
                } else if (AffichageCompteController.compteChoisi.getType().equals("epargne")) {
                    solde = LoginController.gestionnaireGuichet.retraitEpargne(montant,AffichageCompteController.compteChoisi.getNumeroCompte());
                }
                ...
                if (solde != -1 && solde != -2){
                    ...
                    alert.setContentText("Erreur lors du retrait veuillez ressayer");
                    alert.show();
                }else{
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setContentText("Retrait impossible : fonds insufisant");
                    alert.show();
                }
